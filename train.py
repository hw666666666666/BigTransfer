...
# ============================================================================
"""Big Transfer on CIFAR-10 with BiT-M-ResNet101x3"""
...
def train():
    config.train_dataset_path = os.path.join(config.dataset_path, '')
    config.eval_dataset_path = os.path.join(config.dataset_path, '')
    ...
    net = KNOWN_MODELS[model_name](head_size=10, zero_head=True)

    net.load_from(np.load(os.path.join(config.data_path, f"{model_name}.npz")))
    ...
    if config.device_target in ["Ascend", "GPU"]:
        scale_sense = nn.wrap.loss_scale.DynamicLossScaleUpdateCell(loss_scale_value=2 ** 24,
                                                                    scale_factor=2, scale_window=100)
    ...
    # model = ms.Model(net_with_loss, optimizer=opt, metrics=metrics, eval_network=dist_eval_network, amp_level="O0")
